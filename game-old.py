...
class Game(object):
    ...
    def play(self, letter='', ident='', name='', country=''):
        ...
        img = 'Hangman-%d.png' % len(self.missed)
        ...
        if status:
            with sqlite3.connect('lb.sqlite') as conn:
                conn.execute('INSERT INTO games(ident,status,date,name,country) VALUES(?,?,?,?,?);',(ident, status, datetime.now(), name, country))
        return (text, img)
    ...
    def stats(self):
        res = ''
        with sqlite3.connect('lb.sqlite') as conn:
            cur = conn.execute('SELECT name,country,count(id) as c FROM games WHERE status="won" GROUP BY ident ORDER BY c DESC LIMIT 20')
            res = 'Games won:\n'
            res += '\n'.join('%s | %s | %s' % row for row in cur.fetchall())
            cur = conn.execute('SELECT name,country,count(id) as c FROM games WHERE status="lost" GROUP BY ident ORDER BY c DESC LIMIT 20')
            res += '\n\nGames lost:\n'
            res += '\n'.join('%s | %s | %s' % row for row in cur.fetchall())
            cur = conn.execute('SELECT name,country,count(id) as c FROM games WHERE status="hit" GROUP BY ident ORDER BY c DESC LIMIT 20')
            res += '\n\nLetters guessed:\n'
            res += '\n'.join('%s | %s | %s' % row for row in cur.fetchall())
            cur = conn.execute('SELECT name,country,count(id) as c FROM games WHERE status="miss" GROUP BY ident ORDER BY c DESC LIMIT 20')
            res += '\n\nLetters missed:\n'
            res += '\n'.join('%s | %s | %s' % row for row in cur.fetchall())
        return res
